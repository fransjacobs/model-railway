...
ALTER TABLE FUNCTIONS ADD CONSTRAINT FUNC_PK PRIMARY KEY ( LOCO_ID, NUMBER );
ALTER TABLE FUNCTIONS ADD CONSTRAINT FUNC_LOCO_FK FOREIGN KEY ( LOCO_ID ) REFERENCES LOCOMOTIVES ( ID );
...
ALTER TABLE TILES ADD CONSTRAINT TILE_ASSE_SENS_CK CHECK (
  (ACCESSORY_ID IS NOT NULL AND SENSOR_ID IS NULL) OR 
  (ACCESSORY_ID IS NULL AND SENSOR_ID IS NOT NULL)
);

ALTER TABLE TILES ADD CONSTRAINT TILE_ASSE_FK FOREIGN KEY (ACCESSORY_ID) REFERENCES ACCESSORIES(ID);
...
ALTER TABLE ROUTES ADD CONSTRAINT ROUT_FROM_TO_UN UNIQUE (FROM_TILE_ID,TO_TILE_ID);
CREATE UNIQUE INDEX ROUT_FROM_TO_UN_IDX ON ROUTES (FROM_TILE_ID,TO_TILE_ID);

/*
 * Route elements the elements (and the direction etc) of al tile between 2 blocks
 */
DROP TABLE IF EXISTS ROUTE_ELEMENTS CASCADE CONSTRAINTS;
...
ALTER TABLE ROUTE_ELEMENTS ADD CONSTRAINT ROEL_UN UNIQUE (ROUTE_ID,TILE_ID,NODE_ID);
CREATE UNIQUE INDEX ROEL_UN_IDX ON ROUTE_ELEMENTS (ROUTE_ID,TILE_ID,NODE_ID);
