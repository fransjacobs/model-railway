 ...
 */
class PreReserveRouteState extends DispatcherState {
  ...
  boolean searchRoute(Dispatcher dispatcher) {
    ...
    // The final destination is not set so use the current target
...
//      }
  ...
  boolean reserveRoute(Dispatcher dispatcher) {
    ...
    ///ise er ee propertie nodig in de dispatcher?
    ...
    if (turnoutsNotLocked(route)) {
      ...
      dispatcher.showRoute(route, Color.yellow);
