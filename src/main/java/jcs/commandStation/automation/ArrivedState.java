...
class ArrivedState extends AbstractState {
  ...
  void onEnter(Dispatcher dispatcher) {
    ...
    Logger.tag(TAG).debug("Dispatcher {} has arrived in Destination block: {} and {} ...",
            dispatcher.getName(), destinationBlock.getDescription(), (alwaysStop ? "must stop" : "may continue"));
  ...
  AbstractState execute() {
    ...
    if (dispatcher.getNextRouteBean() != null && dispatcher.getNextRouteBean().isLocked()) {
      Logger.tag(TAG).debug("Dispatcher {} is continuing with route: {}",
              dispatcher.getName(), dispatcher.getNextRouteBean().getId());
      ...
      Logger.tag(TAG).debug("Dispatcher {} New departure: {} New destination: {} ...",
              dispatcher.getName(), departureBlock.getId(), destinationBlock.getId());
      ...
      Logger.tag(TAG).debug("Dispatcher " + dispatcher.getName() + " Departure: " + departureBlock.getId() + " Occupancy Sensor: " + occupancySensorId + " Exit Sensor: " + exitSensorId);
      Logger.tag(TAG).debug("Dispatcher " + dispatcher.getName() + " Destination: " + destinationBlock.getId() + " Enter Sensor: " + enterSensorId + " In Sensor: " + inSensorId);
    ...
    Logger.tag(TAG).debug("Dispatcher " + dispatcher.getName() + " Arrived in " + destinationBlock.getDescription() + " next Route " + (nextRoutePrepared ? "is" : "is not") + " prepared. Direction: " + locomotive.getDirection() + (dispatcher.getRouteBean() != null ? " Route: " + dispatcher.getRouteBean().getId() : "") + " Velocity: " + locomotive.getVelocity() + "...");
