...
import jcs.controller.marklin.cs.net.*;
  ...
  private void checkConnection() {
    try {
      if (clientSocket == null || !clientSocket.isConnected()) {
        clientSocket = new Socket(dccExAddress, CSConnection.CS_RX_PORT);
      ...
      Logger.error("Can't (re)connect with Central Station " + dccExAddress.getHostAddress() + ". Cause: " + ex.getMessage());
      Logger.trace(ex);
    }
  }
...
//          }
  ...
  public static void main(String[] a) throws UnknownHostException {
    ...
    //CanMessage m = c.sendMessage(CanMessageFactory.querySystem(uid));
...
//    }
    ...
    //CanMessage m2 = c.sendMessage(CanMessageFactory.querySystem(uid));
