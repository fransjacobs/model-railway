...
public class H2PersistenceService implements PersistenceService {
    ...
    private LocomotiveBean getLocomotiveFunctions(LocomotiveBean locomotive) {
        if (locomotive != null) {
    ...
    public LocomotiveBean getLocomotive(Integer address, DecoderType decoderType) {
        ...
        LocomotiveBean loco = getLocomotiveFunctions(database.where("address=? and decoder_type=?", args).first(LocomotiveBean.class));
        return loco;
    }

    @Override
    public LocomotiveBean getLocomotive(Long id) {
        LocomotiveBean loco = getLocomotiveFunctions(database.where("id=?", id).first(LocomotiveBean.class));
        return loco;
    }
