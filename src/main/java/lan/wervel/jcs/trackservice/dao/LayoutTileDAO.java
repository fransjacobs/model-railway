...
public class LayoutTileDAO extends AbstractDAO<LayoutTile> {
  ...
  private static final String INS_LT_STMT = "insert into layouttiles (TILETYPE,ROTATION,DIRECTION,X,Y,OFFSETX,OFFSETY,SOAC_ID,FEMO_ID,PORT,LTGR_ID,ID) values(?,?,?,?,?,?,?,?,?,?,?,?)";
  private static final String UPD_LT_STMT = "update layouttiles set TILETYPE = ?,ROTATION = ?,DIRECTION = ?,X = ?,Y = ?, OFFSETX = ?,OFFSETY = ?,SOAC_ID = ?,FEMO_ID = ?,PORT = ?,LTGR_ID = ? where ID = ?";
  ...
  protected LayoutTile map(ResultSet rs) throws SQLException {
    ...
    BigDecimal femoId = rs.getBigDecimal("FEMO_ID");
    Integer port = rs.getInt("PORT");
    BigDecimal ltgrId = rs.getBigDecimal("LTGR_ID");

    LayoutTile lt = new LayoutTile(id, tiletype, rotation, direction, x, y, offsetX, offsetY, soacId, femoId, port, ltgrId);
    return lt;
  }
  ...
  protected void bind(PreparedStatement ps, LayoutTile layoutTile) throws SQLException {
    ...
    if (layoutTile.getFemoId() != null) {
      ps.setBigDecimal(9, layoutTile.getFemoId());
    } else {
      ps.setBigDecimal(9, null);
    }
    if (layoutTile.getPort() != null) {
      ps.setInt(10, layoutTile.getPort());
    } else {
      ps.setNull(10, Types.INTEGER);
    }
    if (layoutTile.getLtgrId() != null) {
      ps.setBigDecimal(11, layoutTile.getLtgrId());
    } else {
      ps.setBigDecimal(11, null);
    }

    ps.setBigDecimal(12, layoutTile.getId());
  }
