...
public class SvgIconToPngIconConverterTest {
  ...
  public void setUp() {
    ...
    File functionPath = new File(path);
    File svgPath = new File(svgp);

    File testFileAPath = new File(svgPath + File.separator + "AAAA_fkticon_a_179.svg");
    File testFileBPath = new File(svgPath + File.separator + "BBBB_fkticon_a_17.svg");

    skipTest = !functionPath.exists() && svgPath.exists() && testFileAPath.exists() && testFileBPath.exists();

    if (skipTest) {
      Logger.warn("Skip SVG Conversion test due to missing files");
    }
  ...
  /**
   * Test of convertAndCacheAllFunctionsSvgIcons method, of class SvgIconToPngIconConverter.
   */
  @Test
  public void testConvertAndCacheAllFunctionsSvgIcons1() {
    ...
    if (!skipTest) {
      ...
      try {
        instance.convertAndCacheFunctionImage("AAAA_fkticon_a_179", json, true);
      } catch (IOException e) {
        Logger.error(e);
  ...
  public void testConvertAndCacheAllFunctionsSvgIcons2() {
    ...
    if (!skipTest) {
      ...
      try {
        instance.convertAndCacheFunctionImage("BBBB_fkticon_a_17", json, true);
      } catch (IOException e) {
        Logger.error(e);
        Assert.fail(e.getMessage());
      }
