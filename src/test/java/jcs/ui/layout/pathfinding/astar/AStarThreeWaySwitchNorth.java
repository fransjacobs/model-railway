 ...
 */
public class AStarThreeWaySwitchNorth {

  private final PersistenceTestHelper testHelper;

  public AStarThreeWaySwitchNorth() {
  ...
  @Before
  public void setUp() {
    testHelper.runTestDataInsertScript("layout_three_way_switch_north.sql");
  }
  ...
  public void testFindPath_bk_2m_bk1p() {
    ...
    String expPath = "[bk-2-]->[bk-1+]: bk-2-[bk-2] -> se-3 -> st-3 -> st-1 -> tw-1[GREEN] -> st-8 -> se-8 -> bk-1+[bk-1]";
  ...
  public void testFindPath_bk_1p_bk2m() {
    ...
    String expPath = "[bk-1+]->[bk-2-]: bk-1+[bk-1] -> se-8 -> st-8 -> tw-1[GREEN] -> st-1 -> st-3 -> se-3 -> bk-2-[bk-2]";
    List<Node> path = instance.findPath(fromNodeId, fromSuffix, toNodeId, toSuffix);
    String result = instance.pathToString(path);
